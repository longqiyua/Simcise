...
> Simcise = Simplex × Concise。界面在你不需要它的时候消失，在你需要它的时候生长出来。
...
| `--text-muted` | `#707070` | 弱化文字/占位符（小字仍满足 AA） | 全局统一 |
| `--text-faint` | `#767676` | 极弱文字（仅非关键信息） | Fifo |
...
| 语义 | 色值 | 用途 |
|---|---|---|
| 危险 / 删除 | `#B73A3A` ~ `#FF9A9A` | 破坏性操作、错误状态 |
| 成功 | `#047857` ~ `#58C99A` | 完成、连接成功 |
| 警告 | `#A15C00` ~ `#F5BD67` | 注意、待处理 |
| 信息 | `#2563EB` ~ `#7DB2FF` | 提示、链接 |
...
阴影仅用于增强**实体感**与**呼吸感**，不用于装饰、不用于堆叠层级——装饰性阴影是视觉噪音，禁止。层级靠间距和背景色表达。

**只有 2 级阴影：**

| 级别 | Token | 参考值 | 用途 |
|---|---|---|---|
| Air · 空气感 | `--shadow-air` | `0 1px 3px rgba(26,26,26,0.04), 0 6px 20px -8px rgba(26,26,26,0.05)` | 静态实体的存在感 |
| Lift · 呼吸感 | `--shadow-lift` | `0 2px 6px rgba(26,26,26,0.05), 0 14px 36px -14px rgba(26,26,26,0.10)` | hover 浮起、交互反馈 |

**手法：** 大模糊半径 + 负 spread（-8px / -14px）让阴影径向弥散、边缘消失——轻、透、有空气感。

**活跃态发光：** 需要传递「活跃/选中」信息时，用 `--accent-glow` 强调色发光替代黑色阴影（面积同样遵循 15% 原则）。

**聚焦环与内阴影：** 输入框聚焦用 `0 0 0 3px rgba(accent, 0.1)` 聚焦环；陶瓷质感的顶部内高光（`inset 0 1px 0 rgba(255,255,255,0.7)`）用于「釉面」场景。

**暗色模式阴影：** 暗色模式下黑色阴影不可见，需要加大透明度：

| 级别 | 暗色模式 |
|---|---|
| Air | `0 1px 3px rgba(0,0,0,0.22), 0 6px 20px -8px rgba(0,0,0,0.28)` |
| Lift | `0 2px 6px rgba(0,0,0,0.26), 0 14px 36px -14px rgba(0,0,0,0.42)` |

**原则：**
- Route 项目采用**严格扁平**风格，完全不使用阴影，用 1px hairline 边框替代——这是有意的设计选择，不代表通用规范
- 多层阴影叠加时，最多两层，且模糊半径差距不小于 2 倍（Air/Lift 本身即为两层结构）
...
Simcise 采用**原地完成优先**原则。常规流程使用卡片延展、侧栏或内联编辑；模态只作为安全边界，不作为布局捷径。

**允许使用弹窗：**

- 系统级权限请求或跨应用授权
- 未保存数据即将丢失的阻断提示
- 不可恢复、影响范围大的破坏性操作二次确认
...
- 成就、欢迎、营销和更新说明（不得打断任务）

**弹窗设计原则：**

- 弹窗出现必须有充分理由，且无法用其他方式替代
- 使用原生 `<dialog>` 或等价的可访问实现，设置 `aria-labelledby` / `aria-describedby`
- 打开后焦点进入模态，Tab 不得逃逸；关闭后焦点返回触发源；支持 Escape（不可中断系统流程除外）
- 危险操作按钮必须准确描述后果，例如“永久删除 3 个项目”，不得只写“确定”
...
- 弹窗阴影使用 `var(--shadow-lift)`，不得新增第三套装饰性阴影
...
- 破坏性操作（删除）文字使用 `var(--danger)`，与其他操作项视觉区分
...
- 错误状态：边框切换为 `var(--danger)`，下方出现内联错误文字
...
```
默认 → hover: translateY(-2px) + shadow-lift + border-color → accent
默认 → active: scale(0.98) + shadow-air
...
1. 这个元素在默认状态下是必须出现的吗？
2. 这个操作可以原地完成吗？如果用了模态，是否属于安全、权限或不可逆操作？
3. Accent Color 的使用面积超过 15% 了吗？
4. 动画的时长和缓动与其他模块一致吗？
5. 留白是有节奏的，还是均匀铺开的？
6. 暗色模式下的对比度是否经过校验？
7. 毛玻璃效果是否在性能受限设备上做了降级？
8. 滚动容器是否仍有可发现的滚动线索，而不是无条件隐藏滚动条？
9. 键盘、触屏、减少动画与高对比模式是否都能完成核心任务？
10. 文本、图标和控件边界是否达到对应对比度门槛？
...
- 模态关闭后焦点返回触发源，展开面板关闭后焦点保持在控制按钮
- hover 才出现的控件必须同时支持 `:focus-within`；粗指针设备上默认可见
...
|---|---|
| 正常正文 | 对比度 `4.5:1` |
| 大字（≥24px 常规或 ≥18.66px 粗体） | 对比度 `3:1` |
| 关键图标、焦点环、输入边界 | 对比度 `3:1` |
| 紧凑桌面交互目标 | `36×36px`，相邻目标保留间距 |
| 触屏 / 粗指针交互目标 | `44×44px` |
| 状态表达 | 颜色之外至少再提供文字、图标、形状或位置线索 |

正文、代码、错误信息、订单号等用户可能复制的内容默认允许选择。`user-select: none` 仅用于标题栏、拖拽区、按钮标签等纯控件区域。
...
:root {
  ...
  --color-danger: #B73A3A;        /* 可读危险前景（浅色） */
  --color-success: #047857;       /* 可读成功前景（浅色） */
  --color-warning: #A15C00;       /* 可读警告前景（浅色） */
  --color-info: #2563EB;          /* 可读信息前景（浅色） */

  /* ═══ 圆角（统一大圆角，同屏 ≤ 3 种，最小 8px） ═══ */
  --radius-sm: 8px;               /* 最小圆角 */
  --radius-input: 10px;           /* 输入框 */
  --radius-btn: 12px;             /* 按钮 */
  --radius-card: 14px;            /* 卡片、面板 */
  --radius-xl: 20px;              /* 氛围大卡片（零弹窗，无模态） */
  ...
  /* ═══ 阴影（仅 2 级轻径向，装饰性阴影禁止） ═══ */
  --shadow-air: 0 1px 3px rgba(26,26,26,0.04), 0 6px 20px -8px rgba(26,26,26,0.05);
  --shadow-lift: 0 2px 6px rgba(26,26,26,0.05), 0 14px 36px -14px rgba(26,26,26,0.10);
...
| 层级 | 职责 | 消费规则 |
|---|---|---|
| Reference | 保存原始颜色、尺寸与曲线 | 业务组件禁止直接消费 |
| System | `bg-canvas`、`text-primary`、`accent` 等稳定语义 | 产品主题与品牌适配边界 |
| Component | 按钮、输入、卡片的具体状态 | 仅对应组件内部消费 |

Token 变更必须满足：语义不漂移、Light/Dark 同步、无障碍复测、兼容期有别名、移除有迁移说明。机器可读值以 `tokens.json` 为准，可直接消费的 CSS 以 `tokens.css` 为准。

### 9.2 Route / Yuich 对接边界

- 接入方向固定为 **Simcise → 产品消费方**，不得由 Simcise 反向修改产品实现。
- Route 保持严格扁平：允许 Air / Lift 均为 `none`，层级依赖 hairline 与背景阶差。
- Yuich 保持暖米底色与森林绿品牌：允许 Air / Lift 两级，不引入更多阴影等级。
- 两者共享语义、状态、对比度、键盘和动效降级标准，不要求组件外观完全一致。
- 详细映射、未来迁移顺序与验收清单见 `INTEGRATION.md`。

### 9.3 发布门槛

当前唯一版本为 V1.0。在维护者明确决定发布前，不创建新版本号、不提交、不推送。发布前至少完成：Token 结构校验、断链检查、Light/Dark 视觉检查、键盘路径、触屏目标、减少动画与两款产品适配审查。

---

_Simcise V1.0 · 2026.08.19_
